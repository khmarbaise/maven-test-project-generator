...
class PomFile {
  ...
  def writePomFile (PomFile parentPom) {
    writePomFile(parentPom, null)
  }
  def writePomFile () {
    writePomFile(null, null)
  }
  def writePomFile (PomFile parentPom, modulesPar) {
        ...
        builder.project ( 
            ...
            if (parentPom != null) {
              parent {
                ...
                relativePath( '../' + this.folder.parentFile.toURI().relativize( parentPom.folder.toURI() ).toString())
              }
            } else {
              version(this.version)
            }
...
rootPomFile = new PomFile (folder, "org.test.parent", "reactor-parent", "1.0-SNAPSHOT")

levelParentPom = rootPomFile
(1..1000).each { level ->
  ...
  pf.writePomFile(levelParentPom)
...
rootPomFile.writePomFile( null, levelList)
